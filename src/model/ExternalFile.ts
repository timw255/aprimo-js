...
export interface ExternalFile {
  ...
  label: string;
  /**
   * Dictionary of metadata key-value pairs. Requires `select-externalfile: Metadata` header to be returned.
   */
   ...
   * property in the response, add a header with the name 'select-externalfile' and the value 'Tag' to your request.
   */
  tag: string;
  /**
  ...
  usages: string[];
  /**
   * HAL `_links` for this external file.
   */
...
/**
 * HAL link relations for an {@link ExternalFile}.
 */
export interface ExternalFileLinks {
  /** Self link. */
  self: ApiLink;
  /** Public link to this external file. */
